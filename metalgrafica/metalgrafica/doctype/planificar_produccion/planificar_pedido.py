...
@frappe.whitelist()
def raise_material_requests(alternative_default_warehouse):
	"""
	...
	bom_dict = get_so_wise_planned_qty()
	item_dict = get_raw_materials(bom_dict,0)
...
def get_raw_materials(bom_name,non_stock_item=0, use_multi_level_bom=1, only_raw_materials=0,include_subcontracted=1):
	...
	if self.use_multi_level_bom and self.only_raw_materials and self.include_subcontracted:
		...
		for d in frappe.db.sql("""select fb.item_code,
			...
			group by fb.item_code, fb.stock_uom""", {"bom":bom_name}, as_dict=1):
	...
	item_dict = {}
	make_items_dict(item_dict, item_list)

	return item_dict


def get_subitems(self,bom_wise_item_details, bom, parent_qty, include_sublevel, only_raw, supply_subs,non_stock_item=0, create_material_requests_for_all_required_qty=0):
	items = frappe.db.sql("""
		SELECT
			bom_item.item_code,
	...
	for d in items:
		...
		if include_sublevel and d.default_bom:
			if ((d.default_material_request_type == "Purchase" and d.is_sub_contracted and supply_subs)
				...
				my_qty = 0
				projected_qty = get_item_projected_qty(d.item_code, d.default_warehouse)

...
erpnext.SerialNoBatchSelector = Class.extend({
	...
	make_dialog: function() {
		...
		let fields = [
			...
			{
				fieldname: 'update_total',
				fieldtype:'Button',
				label: __('Actualizar total'),
				click: function(e) {
					me.update_total_qty();
				}
			},
			...
			},
			{
				...
				fieldtype:'Button',
				label: __('Obtener lotes necesarios'),
				click: function(e) {
					//automatic_batch_fill(me.warehouse_details.name, me.item_code);
					frappe.call({
						method: 'erpnext.stock.doctype.batch.batch.get_batches',
						args: {
						...
						callback: (r) => {
				    		...
				    		//me.dialog.fields_dict.qty.set_input(total_qty);
				    		me.update_total_qty();
			...
			{
				fieldname: 'get_batches_all',
				fieldtype:'Button',
				label: __('Obtener todos los lotes'),
				click: function(e) {
					//automatic_batch_fill(me.warehouse_details.name, me.item_code);
					frappe.call({
						method: 'erpnext.stock.doctype.batch.batch.get_batches',
			...
			{fieldtype:'Column Break'},
			{
				...
				fieldtype:'Float',
				read_only: 1,
				label: __(me.has_batch ? 'Cantidad total' : 'Cantidad'),
				default: 0
			},
			{
				...
				fieldtype:'Float',
				read_only: 1,
				label: __('Cantidad requerida'),
				default: me.required_qty
			},
		];
				...
				fields: [
					{
						...
						get_query: function() {
							...
							if(me.warehouse_details.name) filters["warehouse"] = me.warehouse_details.name;
							//console.log(filters);
							//console.log(me);
							return {
...
function automatic_batch_fill(warehouse, item_code) {
	frappe.call({
		method: 'erpnext.stock.doctype.batch.batch.get_batches',
		args: {
			warehouse: warehouse,
